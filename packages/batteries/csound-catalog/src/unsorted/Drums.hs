...
dumbBass = do
  ...
  let
    kfreqenv = expseg [50, 0.01, 200, 0.08, 50]
...
bassDrum cps = env * osc (sig cps * kgliss * 0.5)
  where
    env = linseg [0, 0.00245, 1, 0.1225, 0, 1, 0]
...
hihat kenv = do
  asig <- rand kenv
  let
    a1 = moogvcf (butbp asig cf (cf / 2)) cf 0.7
