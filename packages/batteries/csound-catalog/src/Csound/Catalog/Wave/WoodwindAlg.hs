...
import Csound.Base hiding (fromSpec, select, sustain)
...
rndWoodwind spec vibCoeff attack sustain decay brightnessLevel cps = do
  ...
  let
    (harms1, inorm) = woodwindRange spec durs cps
  ...
  where
    ...
    brightness (iattack, isustain, idecay) level asig = balance (tone asig env) asig
      where
        ifiltcut = tablei (9 * level) (skipNorm $ doubles [40, 40, 80, 160, 320, 640, 1280, 2560, 5120, 10240, 10240])
