...
xanaduPlucks cps vibrAmp vibrCps = (phi vib, phi shift, phi (-shift))
  where
    phi asig = pluck 1 (cpsoct $ oct + asig) cps giwave 1
    shift = 8 / 1200
...
xanadu1 cps = do
  ...
  where
    (g, gleft, gright) = xanaduPlucks cps (1 / 120) (cps / 50)
    f1 = expseg [0.01, 10, 1]
...
xanadu2 cps = do
  ...
  where
    (g, gleft, gright) = xanaduPlucks cps (1 / 80) 6.1
    damping = 1
...
xing xdur cps = asignal
  where
    amps xs dt vib freq phs = ar (loopseg xs (sig $ 1 / xdur)) * (1 + poscil vibEnv freq sine `withD` phs)
      where
        vibEnv = ar $ loopseg [0, dt, vib, sig xdur - dt, 0] (sig $ 1 / xdur)
...
melody xdur cps = do
  k1000 <- randi 1 10
  let
    k100 = cps + loopseg [0, 0.5, 1, sig xdur, 1] (sig $ 1 / xdur) * poscil 1 (5 + k1000) sine
