...
genFreeTog gcat as = source $ do
  ...
  let
    res = groupToggles sum sams $ fmap (snaps . (\i -> ifB (current ==* i) 1 0)) ids
  return (gcat guis, res)
  where
    (names, sams) = unzip as
...
genSimInits gcat numBeats as = source $ do
  ...
  let
    mkReaders bpm =
      zipWithM_
        ( \r ref -> runEvt (syncBpm (bpm / sig (int numBeats)) $ snaps r) $ \x -> do
            writeRef ref (sig x)
        )
        readProcs
        curRefs
  let
    res = bindBpm (\bpm x -> mkReaders bpm >> return x) $ groupToggles mean sams $ fmap snaps currents
  return (gcat guis, res)
  where
    (names, sams, initVals) = unzip3 as
    ids = fmap (sig . int) [1 .. length as]
...
genTogWithRef gcat numBeats as = source $ do
  ...
  let
    mkReaders bpm =
      zipWithM_
        ( \r i -> runEvt (syncBpm (bpm / (sig $ int numBeats)) $ snaps r) $ \x -> do
            when1 (sig x ==* 0 &&* current ==* i) $ do
              writeRef curRef 0
            when1 (sig x ==* 1) $ do
              writeRef curRef i
  ...
  let
    res = bindBpm (\bpm x -> mkReaders bpm >> return x) $ groupToggles sum sams $ fmap (snaps . (\i -> ifB (current ==* i) 1 0)) ids
...
liveEf numBeats names sams masterEff effs = source $ do
  ...
  (gEffs, effCtrls) <-
    fmap unzip $
      mapM (\(tag, initVal) -> slider tag (linSpan 0 1) initVal) $
        zip (replicate n "eff") (fmap fst effs)
  let
    gCtrls = zipWith ctrlGui gEffs gVols
  ...
  return $
    ( hor $ g : gs
    , bindBpm (\bpm asig -> proc bpm >> return asig) $
        mul masterVol $
          appEff (snd masterEff) masterEffCtrl $
            mean $
              zipWith mul vols $
                zipWith (uncurry appEff) (zip (fmap snd effs) effCtrls) sigs
    )
  where
    rows = transpose $ splitRows n sams
    ids = fmap (sig . int) [1 .. length (head rows)]
