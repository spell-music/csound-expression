...
ftgen b1 b2 b3 b4 b5 = fmap ( D . return) $ SE $ join $ f <$> lift (unTab b1) <*> lift (unD b2) <*> lift (unD b3) <*> lift (unD b4) <*> lift (unD b5)
    where f a1 a2 a3 a4 a5 = opcsDep "ftgen" [(Ir,(repeat Ir)),(Ir,[Ir,Ir,Ir,Ir,Ir])] [a1,a2,a3,a4,a5]
...
ftgentmp b1 b2 b3 b4 b5 b6 = fmap ( Tab . return) $ SE $ join $ f <$> lift (unD b1) <*> lift (unD b2) <*> lift (unD b3) <*> lift (unD b4) <*> lift (unD b5) <*> mapM (lift . unD) b6
    where f a1 a2 a3 a4 a5 a6 = opcsDep "ftgentmp" [(Ir,(repeat Ir))] ([a1,a2,a3,a4,a5] ++ a6)
...
sndload ::  Str -> SE ()
sndload b1 = SE $ join $ f <$> lift (unStr b1)
    where f a1 = opcsDep_ "sndload" [(Xr,[Sr,Ir,Ir,Ir,Ir,Ir,Ir,Ir,Ir,Ir])] [a1]