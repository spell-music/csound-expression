...
analogDelay ain kmix ktime kfback ktone =
  liftInternalUdo udoContent "AnalogDelay" rates (ain,kmix,ktime,kfback,ktone)
  where
    rates = [(Ar, [Ar,Kr,Kr,Kr,Kr])]

    udoContent = Text.decodeUtf8 $ $(embedFileRelative ("data" </> "opcodes" </> "MultiFX" </> "AnalogDelay.udo"))
...
pingPongDelay (ain1, ain2) kdelayTime kFeedback kMix kWidth kDamp iMaxDelayTime =
  liftInternalMultiUdo udoContent "StereoPingPongDelay" rates (ain1, ain2, kdelayTime, kFeedback, kMix, kWidth, kDamp, iMaxDelayTime)
  where
