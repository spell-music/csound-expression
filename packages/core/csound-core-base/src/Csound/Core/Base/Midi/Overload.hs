...
instance MidiInstr (Sig -> (Sig, Sig, Sig, Sig)) where
    ...
    onMsg f msg = return $ (toSig (ampmidi 1) * a1, toSig (ampmidi 1) * a2, toSig (ampmidi 1) * a3, toSig (ampmidi 1) * a4)
        where (a1, a2, a3, a4) = f (toSig (cpsmidi ))
