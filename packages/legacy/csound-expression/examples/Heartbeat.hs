...
heartbeat amp = phi amp 0.0024 f12 + phi amp 0.0078 f13
  where
    phi amp dec tab = oscili (sig amp * linseg [1, idur, dec]) 4 tab
...
monoPluck amp cps pick plk = a3
  where
    repluck' freq a = repluck plk (sig amp) freq (sig pick) 0.5 a
    a1 =
      0.5
...
chorusel amp cps rise dec = (ar1, ar2)
  where
    k1 = linen (sig amp) rise idur dec
    k2 = linseg [1, idur, 0]
    k3 = kr $ osc 2
    k4 = kr $ 0.5 * osc 2
    inote = cpspch cps
...
instrPluck (amp, pan) = (sig pan * a, (sig $ 1 - pan) * a)
  where
    a = monoPluck 0.5 (cpspch 8) 0.8 0.3
...
scoPluck =
  sco (onArg instrPluck) $
    del 8 $
      mel $
        take n $
          zipWith
            (\amp pan -> 0.5 *| temp (amp, pan))
            (fmap double $ [0, (v / 40) .. v] ++ repeat v)
            (cycle [0.2, 0.8])
  where
    v = 0.6
    dur = 65
...
scoChorusel =
  ...
  where
    unroll (t, dur, (amp, cps, rise, dec)) = [del t $ str dur $ temp (amp, c, rise, dec) | c <- cps]

scoCrackle =
  sco (onArg instrCrackle) $
    har
      [ str 8 $ temp 100
