...
import Prelude hiding (reads, span)
...
import Csound.Control.Evt (Tick, devt, dropE, evtToSig, listAt, loadbang, snaps2)
import Csound.Control.SE
import Csound.Typed.Gui hiding (height, widget, width)
...
matrixButton name xn yn vals initVal = source $ do
  ...
  let
    val = listAt allVals $ fmap sig ind
  return (gui, val)
  where
    allVals = readMatrix xn yn vals
...
rangeJoySig rangeX rangeY (initValX, initValY) =
  mapSource f $
    ujoy (toRelativeInitVal rangeX initValX, toRelativeInitVal rangeY initValY)
  where
    f (x, y) = (fromRelative rangeX x, fromRelative rangeY y)
