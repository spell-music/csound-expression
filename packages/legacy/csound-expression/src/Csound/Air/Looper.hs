...
genLoop playInstr spec dtBpm times' instrs = do
  ...
  let
    knobGuis = ver [mixKnobGui, preFxKnobGui, postFxKnobGui]

  mapGuiSource (\gs -> hor [knobGuis, sca 12 gs]) $
    joinSource $
      vlift3
        ( \(thr, delEvt) x sils -> do
            -- knobs
            ...
            at smallRoom2 $ sum $ zipWith3 (f delEvt thr x) (zip3 times ids repeatFades) (zip5 mixCoeffs preFxProc preCoeffs postFxProc postCoeffs) $ zip3 delRefs refs instrs
        )
        throughDel
        sw
        sil
  where
    ...
    f delEvt thr x (t, n, repeatFadeWeight) (mixCoeff, preFx, preCoeff, postFx, postCoeff) (delRef, silRef, instr) = do
      ...
      let
        playEcho dId = mul (smooth 0.05 $ ifB dId 1 0) $ mul (smooth 0.1 silVal) $ at (echo (dt * t) (ifB dId repeatFadeWeight 0)) $ ifB dId echoSig 0

      mul mixCoeff $
        mixAt postCoeff postFx $
          sum
            [ return $ sum $ fmap playEcho [d0, d1, d2]
            , playSf 1
            ]
      where
