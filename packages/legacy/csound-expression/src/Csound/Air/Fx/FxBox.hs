...
import Csound.Typed.Opcode (expcurve, scale)
...
import Csound.Air.Misc (ambiEnv, fromMono, saturator)
...
pongy balance delTime fbk tone width = mapSig2 (pingPong' (def{pingPongDamp = absTone, pingPongWidth = width}) delTime fbk balance)
  where
    absTone = scale (expcurve tone 4) 12000 100
...
ambiGuitar thresh dt fbk toneSig drv ain = do
  ...
  return $
    ( adele 0.35 dt fbk toneSig
        . adele 0.5 (dt / 4) (fbk * 1.25) toneSig
        . saturator 0.3
        . tort drv 0.25
    )
      envSig
