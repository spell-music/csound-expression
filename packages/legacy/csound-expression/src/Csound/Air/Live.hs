...
import Data.Colour.SRGB qualified as C
import Data.Default
...
import Csound.Typed.Opcode hiding (delay, gain, integ, mute, space, tone)

import Csound.Typed.Plugins qualified as P
...
genMixer (parentGui, childGui) as = source $ do
  ...
  let
    g =
      parentGui $
        zipWith3
          (\tag slid mute -> childGui [sca 0.8 tag, sca 8 slid, sca 1.1 mute])
          (gMasterTag : gTags)
          (gMaster : gs)
          (gMasterMute : gMutes)
...
fxBox name fx onOff args = source $ do
  ...
  let
    gOff = setFontSize 25 gOff0
  offRef <- newGlobalRef (0 :: Sig)
  writeRef offRef off
  let
    (names, initVals) = unzip args
  ...
  let
    f x = do
      ...
      return res
  let
    gui = setBorder UpBoxBorder $ go (length names) gOff gs
  return (gui, f)
  where
    go n gOff gs
      | n == 0 = gOff
...
uiDistort isOn level drive tone =
  mapSource bindSig $
    sourceColor2 C.red $
      fxBox
        "Distortion"
        (\[level', drive', tone'] -> return . fxDistort level' drive' tone')
        isOn
...
uiChorus isOn mix rate depth width =
  sourceColor2 C.coral $
    fxBox
      "Chorus"
      (\[mix', rate', depth', width'] -> return . stChorus2 mix' rate' depth' width')
      isOn
...
uiFlanger isOn rate depth delay fback =
  mapSource bindSig $
    sourceColor2 C.indigo $
      fxBox
        "Flanger"
        (\[fback', rate', depth', delay'] -> return . fxFlanger fback' rate' depth' delay')
        isOn
...
uiPhaser isOn rate depth freq fback =
  mapSource bindSig $
    sourceColor2 C.orange $
      fxBox
        "Phaser"
        (\[rate', depth', frequency', feedback'] -> return . fxPhaser rate' depth' frequency' feedback')
        isOn
...
uiDelay isOn mix fback time tone =
  mapSource bindSig $
    sourceColor2 C.dodgerblue $
      fxBox
        "Delay"
        (\[mix', fback', time', tone'] -> return . analogDelay mix' fback' time' tone')
        isOn
...
uiEcho isOn maxDelTime time fback =
  mapSource bindSig $
    sourceColor2 C.deepskyblue $
      fxBox
        "Echo"
        (\[time', fback'] -> return . fxEcho maxDelTime time' fback')
        isOn
...
uiFilter isOn lpf hpf gain =
  mapSource bindSig $
    fxBox
      "Filter"
      (\[lpf', hpf', gain'] -> return . fxFilter lpf' hpf' gain')
      isOn
...
uiReverb isOn mix depth =
  sourceColor2 C.forestgreen $
    fxBox
      "Reverb"
      (\[mix', depth'] asig -> return $ cfd mix' asig (rever2 depth' asig))
      isOn
      [("mix", mix), ("depth", depth)]
...
uiWhite isOn freq depth =
  mapSource bindSig $
    sourceColor2 C.dimgray $
      fxBox
        "White"
        (\[freq', depth'] -> fxWhite freq' depth')
        isOn
...
uiPink isOn freq depth =
  mapSource bindSig $
    sourceColor2 C.deeppink $
      fxBox
        "Pink"
        (\[freq', depth'] -> fxPink freq' depth')
        isOn
        [("freq", freq), ("depth", depth)]
...
uiMidi xs initVal = sourceColor2 C.forestgreen $ uiBox "Midi" fx True
  where
    fx = lift1 (\aout arg -> return $ aout + arg) $ vmidiChooser xs initVal
...
genAdsr mkAdsr name b inits = source $ do
  ...
  let
    val = mkAdsr (ir att) (ir dec) (ir sus) (ir rel)
...
uiCompress initThresh initLoknee initHiknee initRatio initAtt initRel initGain =
  mapSource bindSig $
    paintTo orange $
      fxBox
        "Compress"
