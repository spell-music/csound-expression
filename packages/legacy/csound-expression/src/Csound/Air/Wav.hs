...
import Csound.Dynamic hiding (Sco, int)

import Csound.Control.Instr (withDur)
import Csound.Tab (Mp3Chn (..), WavChn (..), mp3Left, mp3s, wavLeft, wavs)
import Csound.Typed
import Csound.Typed.Opcode hiding (metro, pitch, tab, tempo)

import Csound.Control.Evt (loadbang, metro)
...
ramChn isMono n winSize (Phsr file start end speed) pitch =
  ...
  where
    srFactor = sig $ (filesr $ text file) / getSampleRate
