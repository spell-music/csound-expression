...
import Csound.Dynamic hiding (genId, int, pn, when1, whens)
...
mp3s filename skiptime channel =
  preTab
    (SizePlain 0)
    idMp3s
    (FileAccess filename [skiptime, format])
  where
    format = fromIntegral $ fromMp3Chn channel
...
isPowerOfTwo a
  ...
  where
    zeroes = fmap (flip mod 2) $ takeWhile (> 1) $ iterate (\x -> div x 2) a
...
tablewa b1 b2 b3 = fmap (Sig . return) $ SE $ join $ f <$> (lift . unTab) b1 <*> (lift . unSig) b2 <*> (lift . unSig) b3
  where
    f a1 a2 a3 = opcsDep "tablewa" [(Kr, [Kr, Ar, Kr])] [a1, a2, a3]
...
tablew b1 b2 b3 = SE $ join $ f <$> (lift . unSig) b1 <*> (lift . unSig) b2 <*> (lift . unTab) b3
  where
    f a1 a2 a3 = opcsDep_ "tablew" [(Xr, [Xr, Xr, Ir, Ir, Ir, Ir])] [a1, a2, a3]
...
readTab b1 b2 = fmap (Sig . return) $ SE $ join $ f <$> (lift . unSig) b1 <*> (lift . unTab) b2
  where
    f a1 a2 = opcsDep "tab" [(Kr, [Kr, Ir, Ir]), (Ar, [Xr, Ir, Ir])] [a1, a2]
...
readTable b1 b2 = fmap (fromGE . return) $ SE $ join $ f <$> (lift . toGE) b1 <*> (lift . unTab) b2
  where
    f a1 a2 =
      opcsDep
        "table"
...
readTable3 b1 b2 = fmap (fromGE . return) $ SE $ join $ f <$> (lift . toGE) b1 <*> (lift . unTab) b2
  where
    f a1 a2 =
      opcsDep
        "table3"
...
readTablei b1 b2 = fmap (fromGE . return) $ SE $ join $ f <$> (lift . toGE) b1 <*> (lift . unTab) b2
  where
    f a1 a2 =
      opcsDep
        "tablei"
...
tableikt b1 b2 = Sig $ f <$> unSig b1 <*> unTab b2
  where
    f a1 a2 = opcs "tableikt" [(Ar, [Xr, Kr, Ir, Ir, Ir]), (Kr, [Xr, Kr, Ir, Ir, Ir])] [a1, a2]
...
tablekt b1 b2 = Sig $ f <$> unSig b1 <*> unTab b2
  where
    f a1 a2 = opcs "tablekt" [(Ar, [Xr, Kr, Ir, Ir, Ir]), (Kr, [Xr, Kr, Ir, Ir, Ir])] [a1, a2]
...
tablexkt b1 b2 b3 b4 = Sig $ f <$> unSig b1 <*> unTab b2 <*> unSig b3 <*> unD b4
  where
    f a1 a2 a3 a4 = opcs "tablexkt" [(Ar, [Xr, Kr, Kr, Ir, Ir, Ir, Ir])] [a1, a2, a3, a4]
...
cuserrnd b1 b2 b3 = fmap (fromGE . return) $ SE $ join $ f <$> (lift . toGE) b1 <*> (lift . toGE) b2 <*> (lift . unTab) b3
  where
    f a1 a2 a3 =
      opcsDep
        "cuserrnd"
...
duserrnd b1 = fmap (fromGE . return) $ SE $ join $ f <$> ((lift . unTab) b1)
  where
    f a1 =
      opcsDep
        "duserrnd"
...
bpRelativeArgs ys = ArgsPlain $ reader $ \size -> fromRelative size ys
  where
    ...
    substOdds odds xs = zipWith3 go (cycle [True, False]) ((\a -> [a, a]) =<< odds) xs
      where
        go flag odd' x = if flag then odd' else x
