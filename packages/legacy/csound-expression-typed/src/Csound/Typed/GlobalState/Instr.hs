...
import Csound.Typed.GlobalState.Opcodes (exitnow, servantUpdateChnAlive, servantUpdateChnRetrig, turnoff2)
...
saveSourceInstrCachedWithLivenessWatch arity instr = saveInstr $ do
  ...
  livenessWatch arity
  where
    toOut = SE . C.sendChn (arityIns arity) (arityOuts arity)
...
saveSourceInstrCachedWithLivenessWatch_ arity instr =
  saveInstr $
...
saveMidiInstr midiType channel arity instr = do
  ...
  let
    expr = (SE . zipWithM_ (appendVarBy (+) IfKr) vars) =<< instr
