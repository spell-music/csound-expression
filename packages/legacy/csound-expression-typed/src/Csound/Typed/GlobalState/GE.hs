...
import Csound.Dynamic hiding (readMacrosDouble, readMacrosInt, readMacrosString)
import Csound.Dynamic qualified as D (readMacrosDouble, readMacrosInt, readMacrosString)

import Csound.Typed.Constants (infiniteDur)
import Csound.Typed.GlobalState.Cache
import Csound.Typed.GlobalState.Elements hiding (addUdoPlugin, saveNamedInstr)
import Csound.Typed.GlobalState.Opcodes (hrtfmove, hrtfstat, primInstrId)
import Csound.Typed.GlobalState.Options

import Csound.Typed.Gui.Cabbage.Cabbage qualified as Cabbage
import Csound.Typed.Gui.Cabbage.CabbageLang qualified as Cabbage
import Csound.Typed.Gui.Gui (GuiHandle (..), GuiNode, Panel (..), Win (..), defText, guiMap, mapGuiOnPanel, restoreTree)

import Csound.Typed.GlobalState.Elements qualified as E (addUdoPlugin, saveNamedInstr)
...
saveStr = fmap prim . onStringMap . newString
  where
    onStringMap = onHistory stringMap (\val h -> h{stringMap = val})
...
saveToMidiInstr ty chn expr = onMidiMap (saveMidiInstr ty chn expr)
  where
    onMidiMap = modifyHistoryField midiMap (\a h -> h{midiMap = a})

saveMidiCtrl :: MidiCtrl -> GE ()
saveMidiCtrl ma = onMidis $ modify (ma :)
  where
    onMidis = onHistory midiCtrls (\a h -> h{midiCtrls = a})
...
getSysExpr terminatorInstrId = do
  ...
  let
    e2 = event_i $ Event (primInstrId terminatorInstrId) dt 0.01 []
  return $ e1 >> e2
  where
    clearGlobals = snd . renderGlobals

saveAlwaysOnInstr :: InstrId -> GE ()
saveAlwaysOnInstr instrId = onAlwaysOnInstrs $ modify (instrId :)
  where
    onAlwaysOnInstrs = onHistory alwaysOnInstrs (\a h -> h{alwaysOnInstrs = a})
...
onHistory getter setter st = GE $ ReaderT $ \_ -> StateT $ \history ->
  let
    (res, s1) = runState st (getter history)
   in
    return (res, setter s1 history)
...
saveNamedInstr name body = onNamedInstrs $ E.saveNamedInstr name body
  where
    onNamedInstrs = onHistory namedInstrs (\a h -> h{namedInstrs = a})
...
newGuiHandle = modifyWithHistory $ \h ->
  let
    (n, g') = bumpGuiStateId $ guis h
   in
    (GuiHandle n, h{guis = g'})
...
appendToGui gui act = modifyGuis $ \st ->
  st
...
getPanels h = fmap (mapGuiOnPanel (restoreTree m)) $ guiStateRoots $ guis h
  where
    m = guiMap $ guiStateToDraw $ guis h
...
listenKeyEvt evt = do
  ...
  case IM.lookup code keyMap of
    ...
    Nothing -> do
      ...
      hist2 <- getHistory
      let
        newKeyMap = IM.insert code var keyMap
...
hrtfFileNames sr = liftA2 (,) (getDataFileName (name "left" sr)) (getDataFileName (name "right" sr))
  where
    name dir n = concat ["data/hrtf-", show n, "-", dir, ".dat"]
...
readMacrosBy extract allocator name initValue = do
  ...
  return $ extract name
  where
    onMacrosInits = onHistory macrosInits (\val h -> h{macrosInits = val})
...
addUdoPlugin p = onUdo (E.addUdoPlugin p)
  where
    onUdo = onHistory udoPlugins (\val h -> h{udoPlugins = val})
...
getUdoPluginBody name = fmap Text.pack $ readFile =<< getDataFileName filename
  where
    filename = concat ["data/opcodes/", Text.unpack name, ".udo"]
