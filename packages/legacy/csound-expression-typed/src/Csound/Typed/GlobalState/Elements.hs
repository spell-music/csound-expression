...
module Csound.Typed.GlobalState.Elements (
  ...
  -- * Global variables
  Globals (..),
  newPersistentGlobalVar,
  newClearableGlobalVar,
  ...
  Instrs (..),
  saveInstr,
  getInstrIds, -- newInstrId, saveInstrById, saveInstr, CacheName, makeCacheName, saveCachedInstr, getInstrIds,
  ...
  case M.lookup a (idMapContent s) of
    Nothing ->
      let
        ...
        s1 =
          s
            { idMapContent = M.insert a newId (idMapContent s)
...
newTabOfGens = (saveGenId . intTab =<<) . mapM saveGenId
  where
    intTab ns = Gen (nextPowOfTwo $ length ns) (IntGenId (-2)) (fmap fromIntegral ns) Nothing
...
saveGenId a = state $ \s ->
  case M.lookup a (idMapContent s) of
    Nothing ->
      let
        newId = nextReadOnlyTableId $ idMapNewId s
        s1 =
          s
            { idMapContent = M.insert a newId (idMapContent s)
...
newWriteTab = newWriteGen . fromSize
  where
    fromSize n = Gen n (IntGenId 2) (replicate n 0) Nothing
...
renderSf (SfSpec name bank prog) n =
  verbatim $
    Text.unlines
      [ engineStr
      , loadStr
...
saveInstr body = state $ \s ->
  let
    h = hashE body
   in
    case M.lookup h $ instrsCache s of
      Just n -> (n, s)
      Nothing ->
        let
          newId = instrsNewId s
          s1 =
            s
              { instrsCache = M.insert h (intInstrId newId) $ instrsCache s
