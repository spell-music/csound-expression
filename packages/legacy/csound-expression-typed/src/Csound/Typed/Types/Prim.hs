...
import Csound.Dynamic hiding (double, genId, int, str, when1, whens, whileEnd)
import Csound.Dynamic qualified as D
import Csound.Typed.GlobalState.GE
import Csound.Typed.GlobalState.Opcodes (tableI, tableK)
...
class (IsPrim a, RealFrac (PrimOf a), Val a, Floating a) => SigOrD a
...
instance Num Sig where
  (+) = sigOn2 (+) (+)
  (*) = sigOn2 (*) (*)
  negate = sigOn1 negate negate
  (-) = sigOn2 (\a b -> a - b) (\a b -> a - b)
  fromInteger = PrimSig . fromInteger
  abs = sigOn1 abs abs
  signum = sigOn1 signum signum
...
instance Num D where
  (+) = dOn2 (+) (+)
  (*) = dOn2 (*) (*)
  negate = dOn1 negate negate
  (-) = dOn2 (\a b -> a - b) (\a b -> a - b)
  fromInteger = PrimD . fromInteger
  abs = dOn1 abs abs
  signum = dOn1 signum signum
...
instance Floating Sig where
  pi = PrimSig pi
  exp = sigOn1 exp exp
  sqrt = sigOn1 sqrt sqrt
  log = sigOn1 log log
  logBase = sigOn2 logBase logBase
  (**) = sigOn2 (**) (**)
  sin = sigOn1 sin sin
  tan = sigOn1 tan tan
  cos = sigOn1 cos cos
  sinh = sigOn1 sinh sinh
  tanh = sigOn1 tanh tanh
  cosh = sigOn1 cosh cosh
  asin = sigOn1 asin asin
  atan = sigOn1 atan atan
  acos = sigOn1 acos acos
  asinh = sigOn1 asinh asinh
  acosh = sigOn1 acosh acosh
  atanh = sigOn1 atanh atanh
...
instance Floating D where
  ...
  atan = dOn1 atan atan
  acos = dOn1 acos acos
  asinh = dOn1 asinh asinh
  acosh = dOn1 acosh acosh
  atanh = dOn1 atanh atanh
