...
singleOut v0 el = geToSe $ do
  ...
  let
    handleVar = guiHandleToVar handle
...
joy sp1 sp2 (x, y) = geToSe $ do
  ...
  let
    handleVar1 = guiHandleToVar handle1
...
rawBox label = geToSe $ do
  (_, handle) <- newGuiVar
  let
    gui = fromElem [guiHandleToVar handle] [] (Box label)
  appendToGui (GuiNode gui handle) (unSE noInner)
...
setToggle name initVal = sinkSource $ do
  ...
  let
    evtOuts a = outs =<< stepper 0 (fmap sig a)
...
keyIn evt = boolToEvt $ asig ==* 1
  where
    asig = Sig $ fmap (readOnlyVar IfKr) $ listenKeyEvt evt
...
flSetVal trig val handle = SE $ (depT_ =<<) $ lift $ f <$> toGE trig <*> toGE val <*> toGE handle
  where
    f a b c = opcs "FLsetVal" [(Xr, [Kr, Kr, Ir])] [a, b, c]

flPrintk2 :: Sig -> D -> SE ()
flPrintk2 val handle = SE $ (depT_ =<<) $ lift $ f <$> toGE val <*> toGE handle
  where
    f a b = opcs "FLprintk2" [(Xr, [Kr, Ir])] [a, b]
