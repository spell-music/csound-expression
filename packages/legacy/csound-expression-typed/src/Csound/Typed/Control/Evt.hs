...
import Temporal.Media qualified as T (Event (..), render)

import Csound.Dynamic qualified as C
import Csound.Typed.GlobalState.Elements qualified as C

import Csound.Typed.Control.Instr
import Csound.Typed.Control.Mix (Sco)
import Csound.Typed.GlobalState
import Csound.Typed.GlobalState.InstrApi qualified as I
import Csound.Typed.GlobalState.Opcodes (primInstrId)
import Csound.Typed.GlobalState.Port qualified as I
...
renderEvts = fmap (fmap unEvt . T.render)
  where
    unEvt e = (T.eventStart e, T.eventDur e, T.eventContent e)
...
evtLoopGen mustLoop maybeOffEvt instruments evts = apInstr0 $ do
  ...
  where
    ...
    toSingleInstr as = do
      let
        n = mod' (fromE $ getRetrigVal 4) (sig $ int $ length as)
      ref <- newRef 0
    ...
    toSingleEvt es = do
      let
        n = mod' (fromE $ getRetrigVal 4) (sig $ int $ length es)
...
saveEvtInstr_ instrId evts = unSE $ runEvt evts $ \es -> fromDep_ $ mapM_ event es
  where
    event (start, dur, args) = hideGEinDep $ fmap C.event $ C.Event (primInstrId instrId) <$> toGE start <*> toGE dur <*> toNote args
