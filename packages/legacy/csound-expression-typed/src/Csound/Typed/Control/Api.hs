...
import Csound.Typed.GlobalState.Opcodes (Event (..), downsamp, eventi)
...
trigByNameMidi_ name instr = do
  ...
  where
    ...
    go instrId (noteFlag, pch, vol, other) = fromDep_ $ hideGEinDep $ do
      pchExpr <- toGE pch
      let
        instrIdExpr = D.instrIdE instrId + pchExpr / 1000
...
namedMonoMsg name = do
  ...
  tab <- newGlobalTab 24
  let
    onFlag = tabQueue2_hasElements tab
  trigByNameMidiCbk name (onNote tab) (offNote tab)
  when1 onFlag $ do
    let
      (pch, vol) = tabQueue2_readLastElement tab
    writeRef refPch pch
    writeRef refVol vol
