...
rhp (Trip leads (a, b)) = en $ line $ zipWith ($) [triple v3, triple v2, quatra, doub, doub] leads
    where
...
lhp (Trip leads (a, b)) = en $ line $ zipWith ($) [triple v3, triple v2, quatra, quatra]  leads
    where
        triple acc lead = firstLead acc [lead, a, b]  
        quatra     lead = firstLead v2  [lead, a, b, a]

firstLead :: D -> [a] -> Score (D, a)
firstLead accVol (a:as) = lineMap temp $ (accVol, a) : fmap (\x -> (v1, x)) as 
...
firstPart = loop 2 $ chord [leftHand1, rightHand1]

rightHand1 = lineMap rhp' 
        [ t
        , (0, 2, 5)
        , (0, 1, 5)
        , t
...
secondPart = loop 2 $ chord [leftHand2, rightHand2] 

leftHand2 = loop 2 $ lineMap lhp' 
        [ (-2, 1, 2)
        , (-3, 1, 2)]
...
tot = line [loop 2 firstPart, secondPart, firstPart]

res = stretch 1.7 $ sco marimbaSynth $ sustain 0.35 $ loop 4 $ trip2cps $ tot
